...
export default function useCardsHook(cardInfos: {
  [key: string]: any;
}[]) {
  // Create Refs, States and Handlers, Styles then return them.
  const queue = useRef(Array.from(cardInfos, (info) => info['index'])); // backward queue
  const dragStart = useRef({ x: -1, y: -1 });
//   const [props, api] = useSprings(cardInfos.length, (i) => ({
...
//       backgroundImage: `url(${cardInfos[i].preview})`,
